...
################################################################################
# Homassy Deployment Script for VPS (x86_64/AMD64)
#
# This script builds AMD64 Docker images, transfers them to a VPS via SSH,
...
verify_env_file() {
    ...
    local required_vars=("VPS_USER" "VPS_HOST")
    ...
    if [[ -z "$VPS_SSH_KEY" ]] && [[ -z "$VPS_PASSWORD" ]]; then
        error "Either VPS_SSH_KEY or VPS_PASSWORD must be set in .env file"
    fi
...
verify_ssh_connectivity() {
    ...
    if [[ "$USE_SSHPASS" == true ]]; then
        ...
        ssh_cmd="sshpass -e ssh $ssh_base_opts -p $VPS_PORT $VPS_USER@$VPS_HOST"
        scp_cmd="sshpass -e scp $ssh_base_opts -P $VPS_PORT"
        # For rsync, we need to wrap the entire ssh command with sshpass
        rsync_cmd="rsync -e \"sshpass -e ssh $ssh_base_opts -p $VPS_PORT\""
    else
        # SSH key authentication
        if [[ -n "$VPS_SSH_KEY" ]] && [[ -f "$VPS_SSH_KEY" ]]; then
            # Check SSH key permissions
            if [[ "$(uname)" == "Darwin" ]]; then
                local perms=$(stat -f "%OLp" "$VPS_SSH_KEY")
            else
                local perms=$(stat -c "%a" "$VPS_SSH_KEY")
            fi
            ...
            ssh_cmd="ssh $ssh_base_opts -i $VPS_SSH_KEY -p $VPS_PORT $VPS_USER@$VPS_HOST"
            scp_cmd="scp $ssh_base_opts -i $VPS_SSH_KEY -P $VPS_PORT"
            rsync_cmd="rsync -e \"ssh $ssh_base_opts -i $VPS_SSH_KEY -p $VPS_PORT\""
        else
            error "VPS_SSH_KEY is set but file does not exist: $VPS_SSH_KEY"
        fi
    fi

    # Test connection
    if ! eval "$ssh_cmd 'echo SSH connection successful'" &>/dev/null; then
        error "Cannot connect to VPS via SSH at $VPS_USER@$VPS_HOST:$VPS_PORT
Please check:
...
check_remote_disk_space() {
    log "Checking available disk space on VPS..."
...
build_amd64_images() {
    log "Building AMD64 Docker images (native architecture)..."

    # Ensure we're using the default builder (not homassy-builder for ARM64)
    docker buildx use default || warn "Could not switch to default builder"
...
create_remote_directories() {
    log "Creating directory structure on VPS..."
    ...
    eval "$SSH_CMD 'sudo chown -R $VPS_USER:$VPS_USER $DEPLOY_PATH'" || \
...
transfer_docker_images() {
    log "Transferring Docker images to VPS (this may take a while)..."

    for service in "${SERVICES[@]}"; do
        local tar_file="$IMAGE_EXPORT_DIR/${service}.tar"
        local remote_path="$VPS_USER@$VPS_HOST:$DEPLOY_PATH/images/"
...
transfer_env_file() {
    ...
    # Create a deployment-specific .env (filter out SSH credentials)
    local temp_env=$(mktemp)

    # Check if VPS_API_BASE is set
    if [[ -z "$VPS_API_BASE" ]]; then
        error "VPS_API_BASE is not set in .env file. This variable is required for production deployment."
    fi

    # Filter out VPS_ and RASPBERRY_PI_ variables and update NUXT_PUBLIC_API_BASE to production URL
    grep -v "^VPS_" "$SCRIPT_DIR/$ENV_FILE" | \
        grep -v "^RASPBERRY_PI_" | \
        sed "s|NUXT_PUBLIC_API_BASE=.*|NUXT_PUBLIC_API_BASE=$VPS_API_BASE|g" > "$temp_env"

    eval "$SCP_CMD '$temp_env' '$VPS_USER@$VPS_HOST:$DEPLOY_PATH/.env'" || \
...
start_services() {
    ...
    # Pull postgres image (official AMD64 image) with retry logic
    ...
    if [[ -z "$postgres_exists" ]]; then
        ...
        while [[ $retry -lt $max_retries ]]; do
            if eval "$SSH_CMD 'docker pull $POSTGRES_IMAGE'"; then
...
Please check internet connection on VPS and try again.
You can also manually pull the image: ssh $VPS_USER@$VPS_HOST 'docker pull postgres:16'"
                fi
            fi
        done
    else
...
$(eval "$SSH_CMD 'docker logs --tail 50 homassy-web'")"
  ...
  ssh $VPS_USER@$VPS_HOST 'cd $DEPLOY_PATH && docker compose logs'"
}
