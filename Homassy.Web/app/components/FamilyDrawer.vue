  ...
  >
    <template #header>
      <div ref="headerEl" class="flex items-center gap-3 w-full" style="touch-action: none">
        <UIcon name="i-lucide-users" class="h-7 w-7 shrink-0 text-primary-500" />
        <DrawerTitle class="text-xl sm:text-2xl font-semibold">{{ t('profile.family.title') }}</DrawerTitle>
        <DrawerDescription class="sr-only">{{ t('profile.family.title') }}</DrawerDescription>
        <UButton
          class="ml-auto"
          icon="i-lucide-x"
          color="neutral"
...
useDrawerDragToClose(headerEl, {
  ...
  disabled: () => isSubmitting.value || isLeavingFamily.value
})
