<template>
  <div class="relative rounded-2xl overflow-hidden card-animate" style="touch-action: pan-y" data-no-pull-refresh>
    ...
    <!-- Delete confirmation -->
    <AppDrawer :open="isDeleteModalOpen" :title="$t('profile.storageLocations.deleteLocation')" icon="i-lucide-trash-2" fit="content" @update:open="(v) => { isDeleteModalOpen = v }">
      <p class="text-sm text-muted">{{ $t('profile.storageLocations.deleteLocationWarning') }}</p>
      <div class="space-y-2">
        <div>
