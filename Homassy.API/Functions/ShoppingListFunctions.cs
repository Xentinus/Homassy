...
namespace Homassy.API.Functions
{
    public class ShoppingListFunctions
    {
        ...
        public ShoppingListFunctions(FunctionsRuntime runtime, ProductFunctions productFunctions, LocationFunctions locationFunctions, UserFunctions userFunctions, LowStockAutomationFunctions lowStock)
        {
            _runtime = runtime;
        ...
        public ShoppingListItemInfo BuildItemInfo(ShoppingListItem item, ShoppingList list)
        {
            ...
            var product = item.ProductId.HasValue ? _productFunctions.GetProductById(item.ProductId) : null;
            var shoppingLocation = item.ShoppingLocationId.HasValue ? _locationFunctions.GetShoppingLocationById(item.ShoppingLocationId) : null;
            ...
            return new ShoppingListItemInfo
            {
                ...
                Product = _productFunctions.ConvertToProductInfo(product, userId),
                ShoppingLocation = _locationFunctions.ConvertToShoppingLocationInfo(shoppingLocation),
            ...
            try
            {
                ...
                try
                {
                    var itemName = !string.IsNullOrWhiteSpace(shoppingListItem.CustomName) 
                        ...
                        : (productId.HasValue ? _productFunctions.GetProductById(productId.Value)?.Name : null) ?? "Unknown";
                ...
                var info = BuildItemInfo(shoppingListItem, shoppingList);
                await _runtime.ShoppingList.ItemUpsertedAsync(shoppingList.PublicId, info, cancellationToken, actorPublicId);
        ...
        public async Task<ShoppingListItemInfo> UpdateShoppingListItemAsync(Guid publicId, UpdateShoppingListItemRequest request, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var info = BuildItemInfo(trackedItem, shoppingList);
                ...
                if (hasChanges)
                {
                    try
                    {
                        var itemName = !string.IsNullOrWhiteSpace(trackedItem.CustomName)
                            ...
                            : (trackedItem.ProductId.HasValue ? _productFunctions.GetProductById(trackedItem.ProductId.Value)?.Name : null) ?? "Unknown";
        ...
        public async Task DeleteShoppingListItemAsync(Guid publicId, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                try
                {
                    var itemName = !string.IsNullOrWhiteSpace(shoppingListItem.CustomName) 
                        ? shoppingListItem.CustomName 
                        : (shoppingListItem.ProductId.HasValue ? _productFunctions.GetProductById(shoppingListItem.ProductId.Value)?.Name : null) ?? "Unknown";
        ...
        public async Task<ShoppingListItemInfo> QuickPurchaseFromShoppingListItemAsync(QuickPurchaseFromShoppingListItemRequest request, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (!shoppingListItem.ProductId.HasValue)
                {
                    ...
                    var customItemInfo = BuildItemInfo(trackedShoppingListItem, shoppingList);

                    await _runtime.ShoppingList.ItemUpsertedAsync(shoppingList.PublicId, customItemInfo, cancellationToken, actorPublicId);
                ...
                var product = _productFunctions.GetProductById(shoppingListItem.ProductId);
                if (product == null)
                {
                    throw new ProductNotFoundException("Product not found");
                }
                ...
                var shoppingListItemInfo = BuildItemInfo(trackedShoppingListItem, shoppingList);

                await _runtime.ShoppingList.ItemUpsertedAsync(shoppingList.PublicId, shoppingListItemInfo, cancellationToken, actorPublicId);
            ...
            try
            {
                ...
                try
                {
                    var displayName = shoppingListItem.ProductId.HasValue 
                        ? _productFunctions.GetProductById(shoppingListItem.ProductId)?.Name 
        ...
        public async Task<ShoppingListItemInfo> PurchaseShoppingListItemAsync(PurchaseShoppingListItemRequest request, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var info = BuildItemInfo(trackedShoppingListItem, shoppingList);
                await _runtime.ShoppingList.ItemUpsertedAsync(shoppingList.PublicId, info, cancellationToken, actorPublicId);
        ...
        public async Task<List<ShoppingListItemInfo>> CreateMultipleShoppingListItemsAsync(CreateMultipleShoppingListItemsRequest request, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                foreach (var item in request.Items)
                {
                    ...
                    {
                        var product = _productFunctions.GetProductByPublicId(item.ProductPublicId.Value);
                        if (product == null)
                        {
                    ...
                    {
                        var shoppingLocation = _locationFunctions.GetShoppingLocationByPublicId(item.ShoppingLocationPublicId.Value);
                        if (shoppingLocation == null)
                        {
                ...
                foreach (var sli in createdItems)
                {
                    try
                    {
                        var itemName = !string.IsNullOrWhiteSpace(sli.CustomName) 
                            ? sli.CustomName 
                            : (sli.ProductId.HasValue ? _productFunctions.GetProductById(sli.ProductId.Value)?.Name : null) ?? "Unknown";
                ...
                // Notify everyone viewing this list of each new item, and collect the same
                // projection to return.
                var createdItemInfos = new List<ShoppingListItemInfo>(createdItems.Count);
                foreach (var sli in createdItems)
                {
                    var info = BuildItemInfo(sli, shoppingList);
                    await _runtime.ShoppingList.ItemUpsertedAsync(shoppingList.PublicId, info, cancellationToken, actorPublicId);
                    createdItemInfos.Add(info);
                }

                return createdItemInfos;
            }
            catch (Exception ex)
            {
                await transaction.RollbackAsync(cancellationToken);
        ...
        public async Task<List<ShoppingListItemInfo>> QuickPurchaseMultipleShoppingListItemsAsync(QuickPurchaseMultipleShoppingListItemsRequest request, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                foreach (var itemRequest in request.Items)
                {
                    ...
                    {
                        var storageLocation = _locationFunctions.GetStorageLocationByPublicId(itemRequest.StorageLocationPublicId.Value);
                        if (storageLocation == null)
                        {
                ...
                Log.Information($"User {userId.Value} quick purchased {upsertedItems.Count} shopping list items");
                ...
                // Notify everyone viewing the affected list(s) of each purchased item, and collect
                // the same projection to return.
                var results = new List<ShoppingListItemInfo>(upsertedItems.Count);
                foreach (var (list, item) in upsertedItems)
                {
                    var info = BuildItemInfo(item, list);
                    await _runtime.ShoppingList.ItemUpsertedAsync(list.PublicId, info, cancellationToken, actorPublicId);
                    results.Add(info);
                }
            ...
            if (Inited)
            {
                count = _shoppingListItemCache.Values
                    .Where(sli => !sli.PurchasedAt.HasValue &&
                                  ...
                                  (sli.ProductId == null || !_productFunctions.GetProductById(sli.ProductId.Value)?.IsDeleted == true) &&
