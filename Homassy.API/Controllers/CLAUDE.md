...
- **A message can point at things in the app** — a product, a shop, a storage place, a shopping list (`FamilyChatMessageReferences`, max 5 per message). Each row stores the target's **id and a label snapshot**: the id is resolved on every read, so a renamed product reads correctly in a month-old message, and the snapshot is the fallback for a target that no longer resolves for this reader (deleted, or not theirs), which renders as an unavailable chip rather than a blank
- **References are resolved against `SelectValueFunctions`**, the same lists every picker in the app reads. That makes "valid reference" and "thing the sender could have picked" the same statement, and the stored label is the server's own — a client cannot attach a chip naming something it cannot see, nor name it something it is not. An unresolvable id is a **400**
- A message with references may have **no body at all**: pointing at the shop and the milk is a complete thought
...
- `ActiveChanged` carries who is currently **watching** the conversation — the same flag that decides whether a message notifies, so the count the bubble shows and the people a push skips are the same set. Broadcast to the whole group (the caller included, unlike typing: they are part of the answer), on open, close, leave, disconnect, and when the sweep expires one
